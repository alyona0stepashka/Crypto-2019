...
namespace Lab7
{
    public class BlowFish
    {
        ...
        {
            this._randomSource = new RNGCryptoServiceProvider();
        ...
        public string Encrypt_CBC(string pt)
        {
            ...
            return this.ByteToHex(this._initVector) + this.ByteToHex(this.Encrypt_CBC(Encoding.ASCII.GetBytes(pt)));
        }
        ...
        public string Encrypt_CTR(string pt)
        {
            ...
            return this.ByteToHex(this._initVector) + this.ByteToHex(this.Crypt_CTR(Encoding.ASCII.GetBytes(pt), 2));
        }
        ...
        {
            this.Iv = this.HexToByte(ct.Substring(0, 16));
            return Encoding.ASCII.GetString(this.Crypt_CTR(this.HexToByte(ct.Substring(16)), 2)).Replace("\0", "");
        }
        ...
        public byte[] SetRandomIv()
        {
            ...
            this._randomSource.GetBytes(this._initVector);
            this._ivSet = true;
        ...
        private void SetupKey(byte[] cipherKey)
        {
            ...
            //set up the S blocks
            this._bfS0 = SetupS0();
            this._bfS1 = this.SetupS1();
            this._bfS2 = this.SetupS2();
            this._bfS3 = this.SetupS3();

            this._key = new byte[cipherKey.Length]; // 448 bits
            if (cipherKey.Length > 56)
            {
                throw new Exception("Key too long. 56 bytes required.");
            }

            Buffer.BlockCopy(cipherKey, 0, this._key, 0, cipherKey.Length);
            var j = 0;
            for (var i = 0; i < 18; i++)
            {
                var d = (uint) (((this._key[j % cipherKey.Length] * 256 + this._key[(j + 1) % cipherKey.Length]) * 256 +
                                 this._key[(j + 2)                % cipherKey.Length]) * 256 +
                                this._key[(j + 3) % cipherKey.Length]);
                this._bfP[i] ^= d;
            ...
            {
                this.Encipher();
                this._bfP[i] = this._xlPar;
                this._bfP[i + 1] = this._xrPar;
            }

            for (var i = 0; i < 256; i += 2)
            {
                this.Encipher();
                this._bfS0[i] = this._xlPar;
                this._bfS0[i + 1] = this._xrPar;
            }

            for (var i = 0; i < 256; i += 2)
            {
                this.Encipher();
                this._bfS1[i] = this._xlPar;
                this._bfS1[i + 1] = this._xrPar;
            }

            for (var i = 0; i < 256; i += 2)
            {
                this.Encipher();
                this._bfS2[i] = this._xlPar;
                this._bfS2[i + 1] = this._xrPar;
            }

            for (var i = 0; i < 256; i += 2)
            {
                this.Encipher();
                this._bfS3[i] = this._xlPar;
                this._bfS3[i + 1] = this._xrPar;
            }
        }
        ...
        public byte[] Crypt_CTR(byte[] text, int numThreads)
        {
            ...
            for (var i = 0; i < plainText.Length; i += 8)
            {
                for (var x = 0; x < 8; x++) input[x] = (byte) (counter[x] ^ this._initVector[x]);
                Buffer.BlockCopy(plainText, i, block, 0, 8);
        ...
        private byte[] Crypt_CBC(byte[] text, bool decrypt)
        {
            ...
            var iv = new byte[8];
            Buffer.BlockCopy(this._initVector, 0, iv, 0, 8);
            if (!decrypt)
            {
                for (var i = 0; i < plainText.Length; i += 8)
                {
        ...
        private void SetBlock(byte[] block)
        {
            ...
            if (this.NonStandard)
            {
                this._xrPar = BitConverter.ToUInt32(block1, 0);
                this._xlPar = BitConverter.ToUInt32(block2, 0);
            }
            else
            {
                Array.Reverse(block1);
                Array.Reverse(block2);
                this._xlPar = BitConverter.ToUInt32(block1, 0);
                this._xrPar = BitConverter.ToUInt32(block2, 0);
            }
        }
        ...
        private void GetBlock(ref byte[] block)
        {
            ...
            if (this.NonStandard)
            {
                block1 = BitConverter.GetBytes(this._xrPar);
                block2 = BitConverter.GetBytes(this._xlPar);
            }
            else
            {
                block1 = BitConverter.GetBytes(this._xlPar);
                block2 = BitConverter.GetBytes(this._xrPar);
        ...
        {
            this._xlPar ^= this._bfP[0];
            for (uint i = 0; i < Rounds; i += 2)
            {
                this._xrPar = this.Round(this._xrPar, this._xlPar, i + 1);
                this._xlPar = this.Round(this._xlPar, this._xrPar, i + 2);
            }

            this._xrPar = this._xrPar ^ this._bfP[17];

            //swap the blocks
            var swap = this._xlPar;
            this._xlPar = this._xrPar;
            this._xrPar = swap;
        }

        private void Decipher()
        {
            this._xlPar ^= this._bfP[17];
            for (uint i = 16; i > 0; i -= 2)
            {
                this._xrPar = this.Round(this._xrPar, this._xlPar, i);
                this._xlPar = this.Round(this._xlPar, this._xrPar, i - 1);
            }

            this._xrPar = this._xrPar ^ this._bfP[0];
        ...
        {
            var x1 = (this._bfS0[this.WordByte0(b)] + this._bfS1[this.WordByte1(b)]) ^ this._bfS2[this.WordByte2(b)];
            var x2 = x1 + this._bfS3[this.WordByte3(b)];
            var x3 = x2 ^ this._bfP[n];
            return x3 ^ a;
        }
